  ...
  async function exportar() {
    ...
    try {
      ...
      if (fmt === 'csv') {
        ...
        // Procesar filas para PDF (Agrupar por persona)
        // Filtramos las filas que tienen datos de asistencia (fechas) y las de encabezado
        ...
        dataPreview.rows.forEach(row => {
          // Detectar encabezado de persona (ej: 📋 Meylin...)
          if (row[0]?.includes('📋')) {
            const nombre = row[0].replace('📋 ', '').trim()
            const dni = row[1]?.replace('DNI: ', '').trim() || ''
            const rolTurno = row[2]?.split(' · ') || []
            const rol = rolTurno[0] || ''
            const turno = rolTurno[1] || ''
            
            currentPersona = { nombre, dni, rol, turno, asistencias: [] }
            datosPorPersona.push(currentPersona)
          }
          // Detectar fila de datos (fecha válida al inicio)
          else if (row[0]?.match(/^\d{4}-\d{2}-\d{2}$/)) {
            if (currentPersona) {
              currentPersona.asistencias.push(row)
            }
          }
        })
        ...
        datosPorPersona.forEach((persona, index) => {
          ...
          // Limpiar los datos para la tabla (quitar emojis y líneas separadoras)
          ...
          // Total de la persona (Sin emojis)
          ...
          if (totalRow && totalRow[4]) {
            ...
            doc.rect(14, currentY - 4, 60, 6, 'F')
            
            // Texto del total
            doc.text(`Total ${persona.nombre.split(' ')[0]}`, 16, currentY)
            doc.text(`${totalRow[4]}`, 200, currentY)
            currentY += 14
          }
          
          // Salto de página si hay más personas
          if (index < datosPorPersona.length - 1 && currentY > 180) {
            doc.addPage()
            currentY = 20
          }
        })
